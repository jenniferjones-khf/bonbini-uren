...
function berekenDag(inp: any) {
  ...
  // geen toeslagen: geen overuren, geen nacht, geen reistijd, geen zondag, geen
  // turnaround. Kilometers en parkeren zijn geen toeslag maar onkosten, die blijven.
  const toeslagen = inp.otRegelset !== false;
  ...
  return {
    ...
    otUren: toeslagen ? r2(otMin / 60) : 0,
    otE: toeslagen ? r2(otE) : 0,
    nachtUren: toeslagen ? r2(nachtW / 60) : 0,
    naE: toeslagen ? r2(naE) : 0,
    reisuren: r2(R),
    reE: toeslagen ? r2(reE) : 0,
    kmB: r2(kmB),
    zoE: toeslagen ? r2(zoE) : 0,
  };
}
...
async function haalAlles(crewId: string) {
  ...
  return {
    ...
    ik: {
      ...
      otRegelset: crewRec.fields[F.crew.otRegelset] === true,
      gegevens: {
...
async function bewaarDag(crewId: string, body: any) {
  ...
  velden[F.uur.status] = materiaal || kmVeelHoger ? "Afwijking" : "Open";
...
async function dienWeekIn(crewId: string, weeknummer: number) {
  ...
  for (const u of inWeek) {
    ...
    const c = berekenDag({
      ...
      zondag: dag.zondag,
      otRegelset: alles.ik.otRegelset,
    });
    ...
    const ta = alles.ik.otRegelset ? berekenTurnaround(vorige, u, dagprijs) : { uren: 0, bedrag: 0 };
